...
const getTiempo = tiempoDesarrollo => parseFloat ( tiempoDesarrollo.split(" ")[0])

function getMaxTarea (desarrolladores) {
  let maxTiempo = 0;
  let maxTarea = [];
  for (let programador of programadores){
    for (let tarea of programador.tareas){
      if (getTiempo(tarea.tiempoDesarrollo) > maxTiempo ){
        maxTiempo = getTiempo(tarea.tiempoDesarrollo);
        maxTarea= tarea;
      }
    }
  }
...
console.log(getMaxTarea(programadores));
